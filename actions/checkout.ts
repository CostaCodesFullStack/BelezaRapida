...
export async function checkoutAction(data: CheckoutRequest) {
  try {
    ...
    if (!user) {
      return { success: false, error: 'Você precisa estar logado para finalizar a compra.' }
    }
    ...
    if (!address || address.profileId !== profile.id) {
      return { success: false, error: 'Endereço inválido ou não pertence ao usuário.' }
    }
    ...
    for (const item of data.items) {
      ...
      if (!dbProduct) {
        return { success: false, error: `O produto com ID ${item.productId} não foi encontrado.` }
      }

      if (dbProduct.stock < item.quantity) {
        return { success: false, error: `Desculpe, temos apenas ${dbProduct.stock} unidades de "${dbProduct.name}" em estoque.` }
      }
    ...
    if (data.couponId) {
      ...
      if (coupon && coupon.is_active && (!coupon.min_order_value || subtotal >= coupon.min_order_value)) {
    ...
    return { success: false, error: 'Ocorreu um erro interno durante a finalização do seu pedido. Tente novamente.' }
  }
}
